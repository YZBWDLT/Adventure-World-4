...
## 切换为冒险模式
tellraw @a[m=!adventure,hasitem={item=netherbrick,location=slot.weapon.mainhand}] {"rawtext":[{"text":"§c抬头变回创造模式！"}]}
...
## 切换为旁观模式
tellraw @a[m=!spectator,hasitem={item=ender_pearl,location=slot.weapon.mainhand}] {"rawtext":[{"text":"§c抬头变回创造模式！"}]}
