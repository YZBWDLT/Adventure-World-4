...
# --- 令时间流逝 ---
execute @e[name=tick,scores={time=0}] ~~~ scoreboard players remove @e[name=timeline] temp 1
execute @e[name=tick,scores={time=0}] ~~~ scoreboard players remove @e[name=timeline] temp2 1

# --- 成功判定 ---
## 检测到时间结束后，结束关卡
execute @e[name=timeline,scores={temp=..0}] ~~~ function levels/chapter4/4_4/bonus

# --- 召唤怪物 ---
## 当temp2.timeline = 0时代表该生成怪物了

## 倒计时为零后，获取一次难度
execute @e[name=timeline,scores={temp2=0}] ~~~ function lib/get_data/difficulty

## 先指定刷新位置（randomLocation=1~5）
execute @e[name=timeline,scores={temp2=0}] ~~~ scoreboard players random @e[name=randomLocation] data 1 5
...
## 指定刷新位置后指定刷新何种怪物
execute @e[name=timeline,scores={temp2=0}] ~~~ scoreboard players random @e[name=randomMonster] data 1 8
