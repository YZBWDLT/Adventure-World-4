...
## 阻止旁观玩家出界
execute as @a[tag=spectator,scores={deathState=0}] at @s anchored eyes if block ~~80~ barrier positioned -126 6 52 run function aw/lib/events/player_out_of_border
## 当玩家在上一关重生点时，回到本关
execute as @a positioned -117 2 16 if entity @s[r=2] run tp @s -126 6 52
## 阻止玩家关门时跑出房间
execute as @a at @s if block ~~80~ positioned -126 6 52 run function aw/lib/events/player_out_of_border
