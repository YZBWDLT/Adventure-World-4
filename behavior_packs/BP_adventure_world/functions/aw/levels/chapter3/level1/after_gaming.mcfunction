...
execute positioned -172 -20 11 positioned ~-1.2~-1~-1.2 as @e[dx=1.4,dy=3,dz=1.4,type=aw:wind_pearl] run kill @s

# --- 检查玩家进入水迷宫出口 ---
# 在剧情模式下（data.storyMode == 1），当检查到未曾启用过动画（active.timeline == 1），且玩家到达出口后，则开始进行动画
execute if score storyMode settings matches 1 if entity @a[x=-185,y=-21,z=27,dx=4,dy=4,dz=4] if score timeline active matches 1 run function aw/levels/chapter3/level1/events/start_camera_animation

# --- 剧情 ---
# 此处设定 active.timeline 会调用不同对话。
# 当 active.timeline == !2 时，调用传声石结晶的对话
# 当 active.timeline == 2 时，调用额外视角动画的对话
execute if score timeline active matches 2 run function aw/levels/chapter3/level1/events/camera_animation
