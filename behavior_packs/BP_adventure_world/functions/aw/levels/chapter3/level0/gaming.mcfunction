...
## [300] 相机视角：3-3 视角
execute if score timeline time matches 300 run tellraw @a {"rawtext":[{"text":"* §c米云溪： §7你要面临的下一部分，是......【湖泊神殿】。"}]}
execute if score timeline time matches 380 run tellraw @a {"rawtext":[{"text":"* §c米云溪： §7【溺尸携深海守卫者隐于水下，守卫者之魁首镇守于此】"}]}
...
execute if score timeline time matches 860 run tellraw @a {"rawtext":[{"text":"* §f林乐（我）： §7也就是说，我在战斗过程中只能选择一瓶来用？"}]}
## [900] 相机视角：回到玩家视角
execute if score timeline time matches 940 run tellraw @a {"rawtext":[{"text":"* §c米云溪： §7是的，而且这三种药水都会伴随一些副作用，需要你自己来权衡了。"}]}
