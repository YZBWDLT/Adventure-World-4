...
## 阻止旁观玩家出界
execute as @a[scores={deathState=0,spectator=!0}] at @s anchored eyes if block ~~80~ barrier positioned -161 -17 110 run function aw/lib/events/player_out_of_border
## 当玩家在上一关重生点时，回到本关
execute as @a positioned -119 -16 103 if entity @s[r=2] run tp @s -161 -17 110
## 阻止玩家关门时跑出房间
execute as @a at @s if block ~~80~ structure_void positioned -161 -17 110 run function aw/lib/events/player_out_of_border
...
# --- 触发成就 ---
## (1) 无作弊，(2) 成就未获取，(3) 位于(-168,-8,94)的诡异木按钮(方向参数：2)被按下 -> 躲这里不错
execute if score hasCheat data matches 0 if score achievement.findButton2 record matches 0 if block -168 -8 94 warped_button ["facing_direction"=2,"button_pressed_bit"=true] run function aw/lib/achievements/levels/find_button_2
