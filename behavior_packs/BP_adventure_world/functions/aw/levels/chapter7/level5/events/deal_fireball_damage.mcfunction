...
# 若有dealFireballDamage标签，则直接施加伤害（一阶段为 15 伤害，二阶段为 25 伤害）
    execute if score temp.stage data matches 1 run damage @e[tag=dealFireballDamage] 15 override
    execute if score temp.stage data matches 2 run damage @e[tag=dealFireballDamage] 25 override
# 产生虚拟爆炸（粒子、音效、移除火球实体）
