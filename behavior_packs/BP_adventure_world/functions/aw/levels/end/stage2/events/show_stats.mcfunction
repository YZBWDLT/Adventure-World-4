...
# 综合评价（仅单人模式）
    ...
    execute if score playerAmount record matches 1 if score score record matches 106.. run tellraw @a {"rawtext":[{"translate":"评价等级 | §bS \n还是PVE大佬？！！"}]}
    execute if score playerAmount record matches 1 if score score record matches 91..105 run tellraw @a {"rawtext":[{"translate":"评价等级 | §aA \n太赞啦！你的PVE有一手！"}]}
    execute if score playerAmount record matches 1 if score score record matches 76..90 run tellraw @a {"rawtext":[{"translate":"评价等级 | §3B \n不错哦！减少死亡次数或降低时长或许有助于你提高评价！"}]}
    execute if score playerAmount record matches 1 if score score record matches 60..75 run tellraw @a {"rawtext":[{"translate":"评价等级 | §eC \n还可以！也许是时长或死亡严重拖累了你...多练练就好啦~"}]}
    execute if score playerAmount record matches 1 if score score record matches 46..60 run tellraw @a {"rawtext":[{"translate":"评价等级 | §6D \n这个地图可能对你稍微有些挑战性...多练练这张地图来提升你的PVE吧！"}]}
    execute if score playerAmount record matches 1 if score score record matches 31..45 run tellraw @a {"rawtext":[{"translate":"评价等级 | §cE \n您的PVE水平还有待精进呀，勇气贤者sama~"}]}
    execute if score playerAmount record matches 1 if score score record matches 0..30 run tellraw @a {"rawtext":[{"translate":"评价等级 | §4F \n......qwq"}]}
