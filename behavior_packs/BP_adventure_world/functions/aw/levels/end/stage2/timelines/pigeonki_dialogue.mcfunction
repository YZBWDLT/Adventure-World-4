...
# --- 剧情 ---
    execute if score timeline time matches 60 run tellraw @a {"rawtext":[{"translate":"* §a鸽子（PigeonKI）： §7巴豆、量筒和绿叶的配合超级有默契的！有可能的话，我很希望他们能多多合作awa§8(1/2)"}]}
    execute if score timeline time matches 120 run tellraw @a {"rawtext":[{"translate":"* §a鸽子（PigeonKI）： §7偷偷告诉你哟，作者们都是鸽子，7-5的制作拖了一年呢=p§8(2/2)"}]}
