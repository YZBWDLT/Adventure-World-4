...
execute if score timeline time matches 1 if score temp.condition data matches 1 run tellraw @a {"rawtext":[{"translate":"* §a小飞侠（tck1122）： §7恭喜通关！如果感兴趣的话，还可以选择更高的难度进行挑战噢！§8(1/2)"}]}
execute if score timeline time matches 1 if score temp.condition data matches 2 run tellraw @a {"rawtext":[{"translate":"* §a小飞侠（tck1122）： §7恭喜通关最高难度！如果感觉全流程太长的话，可以选择单个关卡来进行游玩噢！§8(1/2)"}]}
execute if score timeline time matches 1 if score temp.condition data matches 3 run tellraw @a {"rawtext":[{"translate":"* §a小飞侠（tck1122）： §7恭喜通关！悄悄的问一下，开创造游玩的感觉怎么样呀(～￣▽￣)～ 如果觉得某一关太难了，可以单独选择它进行练习！§8(1/2)"}]}
...
execute if score timeline time matches 60 run tellraw @a {"rawtext":[{"translate":"* §a小飞侠（tck1122）： §7在试炼中挑战自我固然有趣，不过也不要忘记现实世界中的工作和学习喔！§8(2/2)"}]}
