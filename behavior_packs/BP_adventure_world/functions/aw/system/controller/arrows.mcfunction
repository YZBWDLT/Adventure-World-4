...
# --- 箭实体清除 ---
    ## 箭数量过多（>=50）时，设置 data.allowArrowSupply=1
    ...
    ## 当 data.allowArrowSupply==1 时，移除箭实体
...
# --- 箭补充 ---
    ...
    ## 当 data.allowArrowSupply==1 时，为所有玩家补充箭
        execute as @a if score allowArrowSupply data matches 1 run function aw/lib/events/supply_arrow
