...
execute @e[name=hookshotAmount,scores={stats=2..}] ~~~ execute @e[family=hookshot,tag=!firstHookshot] ~~~ tellraw @p { "rawtext": [ { "text": "§c正在有人使用绳枪哦！请稍后再试。" } ] }
execute @e[name=hookshotAmount,scores={stats=2..}] ~~~ kill @e[family=hookshot,tag=!firstHookshot]
...
execute @e[name=hookshotAmount,scores={stats=0}] ~~~ tag @a[tag=hookshotThrower] remove hookshotThrower
