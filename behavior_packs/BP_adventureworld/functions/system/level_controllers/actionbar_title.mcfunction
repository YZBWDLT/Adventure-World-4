...
### 玩家数为1时，不显示玩家数量
execute @e[name=allPlayersAmount,scores={background=1}] ~~~ function system/level_controllers/actionbar_singleplayer
### 玩家数大于1时，显示玩家数量
execute @e[name=allPlayersAmount,scores={background=2..}] ~~~ function system/level_controllers/actionbar_multiplayer

## --- 已完成的关卡 ---
## temp3.level=1

### 当玩家在正常关卡区域时，显示对应关卡编号 | 7-0时会有一段时间设定为701，此时不应该显示
execute @e[name=level,scores={temp3=1,background=!701}] ~~~ execute @a[scores={temp2=1..,temp3=1..7}] ~~~ titleraw @s actionbar {"rawtext":[{"translate":"§l%%s-%%s §r§f| §a✔ 关卡已完成","with":{"rawtext":[{"score":{"objective":"temp2","name":"@s"}},{"score":{"objective":"temp3","name":"@s"}}]}}]}

execute @e[name=level,scores={temp3=1}] ~~~ execute @a[scores={temp2=1,temp3=0}] ~~~ titleraw @s actionbar {"rawtext":[{"translate":"§l%%s §r§f| §e§l沙 漠 神 殿","with":{"rawtext":[{"score":{"objective":"temp2","name":"@s"}}]}}]}
execute @e[name=level,scores={temp3=1}] ~~~ execute @a[scores={temp2=2,temp3=0}] ~~~ titleraw @s actionbar {"rawtext":[{"translate":"§l%%s §r§f| §a§l丛 林 神 殿","with":{"rawtext":[{"score":{"objective":"temp2","name":"@s"}}]}}]}
execute @e[name=level,scores={temp3=1}] ~~~ execute @a[scores={temp2=3,temp3=0}] ~~~ titleraw @s actionbar {"rawtext":[{"translate":"§l%%s §r§f| §b§l湖 泊 神 殿","with":{"rawtext":[{"score":{"objective":"temp2","name":"@s"}}]}}]}
