...
### 每刻为变量time.monsterSummonDelay -= 1 | 除去level ID=641~701时
execute @e[name=level,scores={temp3=0,background=!641..701}] ~~~ scoreboard players add @e[name=monsterSummonDelay,scores={time=0..}] time -1
### 调用相关生成器并循环执行
execute @e[name=level,scores={temp3=0,background=!641..701}] ~~~ execute @e[name=wave,scores={background=!0}] ~~~ execute @e[name=monsterSummonDelay,scores={time=0..}] ~~~ function methods/monsters/summoner_controller
