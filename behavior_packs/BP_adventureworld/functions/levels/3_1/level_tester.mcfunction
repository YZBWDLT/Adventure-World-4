...
execute @e[name=level,scores={background=312}] ~~~ execute @a[tag=!playing,c=1] ~~~ function levels/3_1/bonus
execute @e[name=level,scores={background=311}] ~~~ execute @a[tag=!playing] ~~~ function levels/3_1/start_wave2

# -----失败判定-----

## 对在重生点的玩家移除alive标签
execute @a[x=-192,y=-21,z=38,dx=2,dy=3,dz=2] ~~~ scoreboard players set @s isAlive 0

## 当不存在标签为alive的玩家时，游戏失败
execute @e[name=alivePlayersAmount,scores={background=0}] ~~~ function levels/3_1/game_lose
