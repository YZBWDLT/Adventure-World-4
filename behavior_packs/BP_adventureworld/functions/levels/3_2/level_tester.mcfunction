...
execute @e[name=level,scores={background=323}] ~~~ execute @a[tag=!playing,c=1] ~~~ function levels/3_2/bonus
execute @e[name=level,scores={background=321}] ~~~ execute @a[tag=!playing] ~~~ function levels/3_2/start_wave2
execute @e[name=level,scores={background=322}] ~~~ execute @a[tag=!playing] ~~~ function levels/3_2/start_wave3

# -----失败判定-----

## 对在重生点的玩家移除alive标签
execute @a[x=-183,y=-16,z=20,dx=2,dy=3,dz=2] ~~~ scoreboard players set @s isAlive 0

## 当不存在标签为alive的玩家时，游戏失败
execute @e[name=alivePlayersAmount,scores={background=0}] ~~~ function levels/3_2/game_lose
