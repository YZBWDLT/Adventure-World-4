...
### 剩余0秒时，根据阶段上涨层数
execute @e[name=level,scores={background=640}] ~~~ execute @e[name=lavaWarningLeft,scores={time=0}] ~~~ fill -89 -40 49 -61 -40 21 minecraft:lava[] keep
### 岩浆溢出5秒后，收回岩浆并重新计时
execute @e[name=lavaWarningLeft,scores={time=-10}] ~~~ fill -89 -40 49 -61 -40 21 air[] replace minecraft:lava[]
...
execute @a[scores={isAlive=0..1},c=1] ~~~ execute @e[name=monsterAmount,scores={background=0}] ~~~ scoreboard players set @e[name=level] background 641
...
## 当玩家结束战斗后给予奖励
execute @e[name=level,scores={background=641}] ~~~ execute @a[scores={isAlive=2},c=1] ~~~ function levels/chapter6/6_4/bonus

# -----失败判定-----

## 对在重生点的玩家移除alive标签
execute @a[x=-76,y=-30,z=34,dx=2,dy=3,dz=2] ~~~ scoreboard players set @s isAlive 0
