...
execute @e[name=level,scores={background=333}] ~~~ execute @a[tag=!playing,c=1] ~~~ function levels/3_3/bonus
execute @e[name=level,scores={background=331}] ~~~ execute @a[tag=!playing] ~~~ function levels/3_3/start_wave2
execute @e[name=level,scores={background=332}] ~~~ execute @a[tag=!playing] ~~~ function levels/3_3/start_wave3

# -----失败判定-----

## 对在重生点的玩家移除alive标签
execute @a[x=-164,y=-17,z=28,dx=2,dy=3,dz=2] ~~~ scoreboard players set @s isAlive 0

## 当不存在标签为alive的玩家时，游戏失败
execute @e[name=alivePlayersAmount,scores={background=0}] ~~~ function levels/3_3/game_lose
