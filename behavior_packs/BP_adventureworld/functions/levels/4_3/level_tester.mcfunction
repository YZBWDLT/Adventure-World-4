...
execute @e[name=level,scores={background=434}] ~~~ execute @a[tag=!playing,c=1] ~~~ function levels/4_3/bonus
execute @e[name=level,scores={background=431}] ~~~ execute @a[tag=!playing] ~~~ function levels/4_3/start_wave2
execute @e[name=level,scores={background=432}] ~~~ execute @a[tag=!playing] ~~~ function levels/4_3/start_wave3
execute @e[name=level,scores={background=433}] ~~~ execute @a[tag=!playing] ~~~ function levels/4_3/start_wave4

# -----失败判定-----

## 对在重生点的玩家移除alive标签
execute @a[x=-152,y=-8,z=-6,dx=2,dy=3,dz=2] ~~~ scoreboard players set @s isAlive 0

## 当不存在标签为alive的玩家时，游戏失败
execute @e[name=alivePlayersAmount,scores={background=0}] ~~~ function levels/4_3/game_lose
