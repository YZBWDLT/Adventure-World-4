...
execute @e[name=level,scores={background=413}] ~~~ execute @a[tag=!playing,c=1] ~~~ function levels/4_1/bonus
execute @e[name=level,scores={background=411}] ~~~ execute @a[tag=!playing] ~~~ function levels/4_1/start_wave2
execute @e[name=level,scores={background=412}] ~~~ execute @a[tag=!playing] ~~~ function levels/4_1/start_wave3

# -----失败判定-----

## 对在重生点的玩家移除alive标签
execute @a[x=-157,y=16,z=-24,dx=2,dy=3,dz=2] ~~~ scoreboard players set @s isAlive 0

## 当不存在标签为alive的玩家时，游戏失败
execute @e[name=alivePlayersAmount,scores={background=0}] ~~~ function levels/4_1/game_lose
