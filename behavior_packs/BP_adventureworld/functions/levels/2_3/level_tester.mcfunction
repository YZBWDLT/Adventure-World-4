...
execute @e[name=level,scores={background=234}] ~~~ execute @a[tag=!playing,c=1] ~~~ function levels/2_3/bonus
execute @e[name=level,scores={background=233}] ~~~ execute @a[tag=!playing] ~~~ function levels/2_3/start_wave4
execute @e[name=level,scores={background=232}] ~~~ execute @a[tag=!playing] ~~~ function levels/2_3/start_wave3
execute @e[name=level,scores={background=231}] ~~~ execute @a[tag=!playing] ~~~ function levels/2_3/start_wave2

# -----失败判定-----

## 对在重生点的玩家移除alive标签
execute @a[x=-155,y=-22,z=45,dx=2,dy=3,dz=2] ~~~ scoreboard players set @s isAlive 0

## 当不存在标签为alive的玩家时，游戏失败
execute @e[name=alivePlayersAmount,scores={background=0}] ~~~ function levels/2_3/game_lose
